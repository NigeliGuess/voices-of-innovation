...
function useParallax(props) {
  ...
  (0, import_react.useEffect)(() => {
    ...
    return () => {
      if (newElement)
        controller === null || controller === void 0 || controller.removeElementById(newElement.id);
    };
  }, []);
  (0, import_react.useEffect)(() => {
    if (element)
...
var ParallaxBanner = (props) => {
  ...
  function renderLayers() {
    if (targetElement) {
      if (layers && layers.length > 0)
        return layers.map((layer, i) => import_react.default.createElement(ParallaxBannerLayer, {
          ...layer,
          targetElement,
          key: `layer-${i}`,
          testId: `layer-${i}`
  ...
  function renderChildren() {
    if (targetElement)
      return import_react.default.Children.map(props.children, (child) => {
        const item = child;
        if ((item === null || item === void 0 ? void 0 : item.type) === ParallaxBannerLayer)
          return import_react.default.cloneElement(item, { targetElement });
        return child;
      });
    return null;
  }
